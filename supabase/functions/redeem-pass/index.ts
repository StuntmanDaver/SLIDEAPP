...
serve(async (req: Request) => {
  ...
  try {
    ...
    const targetUserId = updatedPass.owner_user_id || updatedPass.issuer_user_id;
    if (targetUserId) {
      ...
      if (token?.expo_push_token) {
        const message: ExpoPushMessage = {
          ...
          title: "Pass redeemed",
          body: "Welcome! Your pass has been redeemed.",
          sound: "default",
          priority: "high",
          data: {
            type: "pass_redeemed",
        ...
        sendExpoPushNotifications([message]).catch((err) => {
